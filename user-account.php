...
<script>
  ...
  jQuery('#student-registration').on('submit',function(){
    ...
    if(true)
    {
      ...
      jQuery.ajax({
        type: "post",
        url: "http://localhost/scotspine education system/actions/student-registration.php",
        data: formdata,
        dataType : 'json',
        beforeSend: function(){
          jQuery('#loader').show();
        ...
        success: function (response) {
          ...
          {
            location.href = 'http://localhost/scotspine education system/admin/user-account.php?/user=student&action=fee-payment&std_id='+response.std_id+'&payment_method='+response.payment_method;
          }
        },
        complete: function(){
          // jQuery('#loader').hide();
